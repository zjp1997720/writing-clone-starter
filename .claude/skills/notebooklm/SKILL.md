...
## Visual Generation Hard Rule

When the task is a visual generation task, Claude MUST append the following suffix to the user's original description / instructions / prompt before calling NotebookLM:

```text
文本字体锐利清晰，4K 超清。中文文字尽量使用常见字，字号尽量大，层级清楚，排版简洁，高对比背景，避免大段密集小字。
```

Apply this hard rule to these visual tasks only:
- `notebooklm generate slide-deck`
- `notebooklm generate video`
- `notebooklm generate infographic`
- `notebooklm generate revise-slide`

Do NOT append this suffix to these non-visual tasks by default:
...
Exception: if the user explicitly asks for an intentionally blurry, lo-fi, retro, grainy, low-resolution, or hazy visual style, do not force the clarity suffix.

Prompt construction rule:
- Preserve the user's original intent first
- Append the suffix at the end, separated by a sentence break
- For Chinese text-heavy slides or visuals, prioritize readability over decorative density
