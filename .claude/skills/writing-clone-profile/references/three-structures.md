...
7) Article Archetype（{selected_profile_dir}/article-archetypes.md；如果存在）
8) Opening / Question / Transition Moves（{selected_profile_dir}/opening-question-transition-moves.md；如果存在）
9) Anti-patterns（{selected_profile_dir}/anti-patterns.md；如果存在）

任务：
- 按指定结构生成一篇公众号风格中文文章。
- 结构 A/B/C 只决定入口与局部展开方式；整篇文章必须服从已选 `article_archetype` 的主骨架。
...
- 如果 profile 提供了开头 / 疑问 / 转场动作库，只能借功能动作，不要机械堆口头禅。
- 主动避开 profile `anti-patterns.md` 中的坏味道。
