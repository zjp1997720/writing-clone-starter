...
7. `{selected_profile_dir}/anti-ai-mechanics.md`：反 AI 味机制（可选；活人感、主线脉冲、体感记忆）
8. `{selected_profile_dir}/article-archetypes.md`：文章原型库（可选；Profile 专属的整篇叙事骨架）
9. `{selected_profile_dir}/opening-question-transition-moves.md`：开头 / 疑问句 / 转场动作库（可选）
10. `{selected_profile_dir}/anti-patterns.md`：Profile 专属坏味道与修法（可选）
11. `{selected_profile_dir}/rewrite-delta-examples.md`：真实改稿差异样本（可选；用于二稿、去 AI 味、人工改稿回流）
12. `{selected_profile_dir}/source-quality-notes.md`：素材质量说明（可选；维护和审计用，不默认当正文素材）

一句话：`style-profile.md + form-normalized-anchors/ + voice-cues.md` 负责前台写稿；Profile 专属二层资产负责原型、节奏和反模式校准；`voice-anchors.md` 负责后台真值与审计。
...
- Human-only Brief Slots → `01_项目/内容创作/{YYMMDD_项目名}/01-Context/human-only-brief-slots.md`
- Article Gravity Card → `01_项目/内容创作/{YYMMDD_项目名}/01-Context/article-gravity-card.md`
- Reader Capture Audit → `01_项目/内容创作/{YYMMDD_项目名}/01-Context/reader-capture-audit.md`
...
- AI Baseline → `01_项目/内容创作/{YYMMDD_项目名}/03-Reviews/草稿_final.ai-baseline.md`
...
- Momentum Map → `01_项目/内容创作/{YYMMDD_项目名}/03-Reviews/Momentum-Map.md`
- Lint Report → `01_项目/内容创作/{YYMMDD_项目名}/03-Reviews/Lint-Report.md`
- Human Feel Review → `01_项目/内容创作/{YYMMDD_项目名}/03-Reviews/Human-Feel-Review.md`
- Gate Check Report → `01_项目/内容创作/{YYMMDD_项目名}/03-Reviews/Gate-Check-Report.md`
- 人工改稿 diff 复盘 → `01_项目/内容创作/{YYMMDD_项目名}/03-Reviews/人工改稿-diff复盘.md`
...
- `first_hand_scene`: 亲自做过 / 见过 / 聊过 / 踩过的具体场景
- `emotion_nodes`: 兴奋、焦虑、愤怒、释然、吐槽等具体时刻
- `human_only_material`: 不能由 AI 编造、必须来自人的素材
- `god_move_candidate`: 非线性类比 / 意象 / 创意嫁接候选（AI 可补候选，但不能冒充作者灵感）
- `cannot_invent`: 明确禁止编造的体验、数据、反馈、项目结果或承诺
...
### Phase 0.1：Human-only Brief Slots（防编造）

参考 `references/human-only-brief-slots.md`，把人类不可替代素材单独写入：

- `01-Context/human-only-brief-slots.md`

规则：

- 缺 `first_hand_scene` 时，可以继续写方法论 / 判断 / 工具说明，但必须标记低在场感风险。
- 缺 `emotion_nodes` 时，不允许用夸张情绪词伪造体感。
- 缺 `god_move_candidate` 时，AI 可以生成类比候选，但不能写成作者的真实顿悟瞬间。
- `cannot_invent` 中列出的内容不得被写成确定事实；必须降级、删除或追问。
...
5. `{selected_profile_dir}/anti-ai-mechanics.md`（如果存在，读取；用于活人感、主线脉冲和节奏治理）
6. `{selected_profile_dir}/article-archetypes.md`（如果存在，读取；用于文章级原型路由）
7. `{selected_profile_dir}/opening-question-transition-moves.md`（如果存在，读取；用于开头、疑问句节奏和口语转场）
8. `{selected_profile_dir}/anti-patterns.md`（如果存在，读取；用于 Lint 和 Human Feel Review）
9. `{selected_profile_dir}/examples.md`（可选：案例锚点）
10. `{selected_profile_dir}/quotes.md`（可选：金句锚点）
11. 用户当前对话和附件素材（必须）
12. **nmem 记忆搜索**（必须，见下）
13. Goldmine Pack（从 `02_素材库/` 抽取少量高相关碎片，按需，见下）
14. 用户仓库内与主题相关内容（必须；优先用 Obsidian 官方 CLI `search:context`，失败回退 Grep）
15. **WebSearch 联网调研**（按需，见下）
...
```bash
obsidian search:context vault=<your_vault_name> query="关键词" path="01_项目" limit=3 format=json

# 当 query 含 ':' 或形如 '#tag:' 时，必须整体加引号做字面量检索
obsidian search:context vault=<your_vault_name> query="\"#due:\"" path="04_系统" limit=3 format=json
```
...
- `01-Context/human-only-brief-slots.md` 已写入
- `01-Context/article-gravity-card.md` 已写入
- `01-Context/reader-capture-audit.md` 已写入
- 内容不是空壳，且足以支撑 `references/context-sufficiency.md` 的评分

对高事实密度、强风格复现、强案例依赖任务，这条硬前置不放松。

### Phase 0.7：Article Gravity Gate（写前吸引力判断）

参考 `references/article-gravity-gate.md`，在 Gate A 前生成：

- `01-Context/article-gravity-card.md`

它判断的不是「上下文够不够」，而是「这篇文章是否值得读者继续读」。

检查槽位：

- `reader_promise`：读者点开后能拿走什么
- `curiosity_hook`：第一屏的悬念 / 冲突 / 反常识 / 具体结果
- `knowledge_gain`：新信息 / 新方法 / 新框架 / 新判断
- `resonance_point`：读者处境或情绪共鸣
- `personal_stake`：作者为什么关心
- `specificity`：是否足够具体

决策规则：

- `gravity_score >= 70`：继续进入 Gate A。
- `50 <= gravity_score < 70`：只修最弱槽位；如果需要补料，只问 1 个高杠杆问题。
- `gravity_score < 50`：先收窄选题或改入口，不要硬写完整稿。

### Phase 0.8：Reader Capture Audit（首屏抓力与内容值检查）

参考 `references/reader-capture-audit.md`，在 Gate A 前生成：

- `01-Context/reader-capture-audit.md`

它不替代 `Article Gravity Gate`。

它补的是更偏内容编辑的视角：不看标题，只看第一屏，读者为什么继续读，作者凭什么值得信。

必须检查：

- `topic`：第一屏到底在讲什么
- `first_screen_promise`：不看标题，读者也能知道自己能带走什么
- `hook_type`：这次是结果、冲突、问题、反差、工具还是场景
- `credibility_source`：作者为什么有资格说这件事
- `material_signals`：数据 / 故事 / 金句 / 权威 / 痛点 / 具体结果 至少命中几类
- `perceived_value`：读者花这几分钟，换到了什么
- `cognitive_gap`：这篇相对常见写法，到底新在哪
- `entry_sync`：标题、摘要、第一屏是不是同一个价值单元
- `cta_focus`：结尾最终只想要读者做哪一个动作

决策规则：

- `decision = continue`：继续进入 Gate A。
- `decision = stress_test_opening`：先做一次 `Opening Stress Test`，再进入 Gate A。
- `decision = ask_one_question`：只问 1 个高杠杆问题，优先补 `credibility_source / cognitive_gap / material_signals`。
- `decision = narrow_angle`：先收窄角度或改第一屏承诺，不要直接进入正文。
...
### Phase 2.3：文章原型路由（Profile 专属，按需）

如果当前 Profile 存在 `{selected_profile_dir}/article-archetypes.md`，必须在正式写骨架前先判断 `article_archetype`。

规则：

- 文章原型解决「整篇文章怎么推进」，A/B/C 结构解决「用什么入口切入」；两者不能混为一谈。
- 只允许选 1 个主原型；另一个命中原型只能作为局部模块，不允许硬拼两套骨架。
- 原型判断必须写入 `03-Reviews/Structure-Eval.md`，至少包含：
  - `article_archetype`
  - `matched_signals`
  - `reader_promise`
  - `archetype_rhythm`
...
2. `候选结构评估`：先根据 `{selected_profile_dir}/article-archetypes.md`（如果存在）确定文章级原型，再参考 `references/three-structures.md` 列出 3 种候选入口结构的骨架（每种只写 3-5 行要点），评估各自与当前主题/素材的适配度、信息密度、重复风险，选定最优结构并说明理由，**必须写入** `03-Reviews/Structure-Eval.md`
3. `Module Budget`：把全文先压成 4 个以内主模块，并为每个模块写清，**必须写入** `03-Reviews/Module-Budget.md`
   - 这个模块只负责什么
   - 它提供的新增信息是什么
   - 如果删掉它，文章会损失什么
4. `Reader Momentum Map`：参考 `references/reader-momentum-map.md`，写清读者为什么会继续读，**必须写入** `03-Reviews/Momentum-Map.md`
   - 当前文章原型是什么，节奏风险是什么
   - 第一屏欠读者什么问题
   - 全文主线是什么
   - 允许哪些 detour，以及如何扣回主线
   - 多案例 / 多工具 / 多观点如何递进
   - 开头埋什么 callback，结尾如何响
5. `Goldmine Retrieval Plan`：只针对 `slot_gap_map` 里真的缺口的槽位写明：
   - 要不要用 Goldmine
   - 用哪个强度档位
   - 每个槽位最多补几条
   - 哪些命中必须丢弃
6. `Opening Stress Test`（条件触发）：如果 `01-Context/reader-capture-audit.md` 的 `decision = stress_test_opening`，或 `first_screen_promise / hook_type / credibility_source / entry_sync` 任一项明显偏弱，必须先生成 3 个开头候选（只允许 3 个，不铺满），并写入 `03-Reviews/Structure-Eval.md`：
   - 每个候选都要标注 `hook_type`
   - 每个候选都要说明 `credibility_source`
   - 每个候选都要说明它承诺读者带走什么
   - 选 1 个进入正文，其他 2 个只作为备选，不继续膨胀成 10+ 方案
...
- 每个知识 / 案例 / 类比 detour 后都必须有 `Main-thread Pulse`；连续 3-4 段没有扣回主线，默认标记为心流风险。
- 如果开头埋了具体场景、人物、物件、问题或句子，结尾优先 callback；不 callback 时在 Momentum Map 里说明原因。
- 如果 `entry_sync` 弱，优先重写标题 / 摘要 / 第一屏的承诺关系，而不是先补中段内容。
...
- `{selected_profile_dir}/anti-ai-mechanics.md`（如果存在）
- `{selected_profile_dir}/article-archetypes.md`（如果存在）
- `{selected_profile_dir}/opening-question-transition-moves.md`（如果存在）
- `{selected_profile_dir}/anti-patterns.md`（如果存在）
...
- `article-archetypes.md` 负责整篇叙事骨架，不负责具体句子模仿
- `opening-question-transition-moves.md` 负责开头、疑问句节奏和自然转场，不作为口头禅词库硬套
- `anti-patterns.md` 负责审稿时识别坏味道，不替代事实核查和 Claim Ledger
- `voice-anchors.md` 保留在后台，不默认作为本轮写稿全文输入
- `reader-capture-audit.md` 负责校准第一屏承诺、Hook、可信度、感知价值和 CTA 单点，不负责替代结构或事实判断
...
- `momentum_risks`（最可能让读者断流的 1-3 个位置）
...
#### Step 3.5：Human Feel & Flow Review

四重 Lint 通过后，参考 `references/human-feel-flow-review.md` 执行活人感与心流终审，并写入：

- `03-Reviews/Human-Feel-Review.md`

重点检查：

1. 情绪是否来自体感、动作、停顿或场景，而不是抽象情绪标签。
2. 是否有大鹏自己的经历、判断、犹豫或取舍。
3. 是否出现连续 3 段以上平稳输出，没有情绪、问题、转向或主线回扣。
4. 每个知识 / 案例 / 类比 detour 是否扣回主线。
5. 可控不完美是否自然存在；不允许为了像人而硬塞口头禅、自嘲或犹豫。
6. 是否命中 `{selected_profile_dir}/anti-patterns.md` 中的坏味道，特别是抽象情绪、方法论悬空、平铺案例、营销前置和结尾不推进。

结果规则：

- `PASS`：进入 Phase 4。
- `REVISE_MODULE`：只重写失败模块。
- `REBUILD_ANGLE`：回到 Article Gravity Card / Momentum Map 重建入口和主线，不直接润色。
...
4. `Reader Momentum Map`（见 `references/reader-momentum-map.md`，共享文章原型、主线、detour 预算、扣主线句和 callback 设计，写入 `03-Reviews/Momentum-Map.md`）

规则：

- 任何可被反驳的事实断言，都必须在 Claim Ledger 里有来源；无来源则降级为主观判断。
- 三子稿共享同一份 `Module Budget`，不允许每个子稿偷偷扩成 5-6 个大模块。
- 三子稿共享同一份 `Reader Momentum Map`，不允许各自发明不同主线。
...
- 不做句子级拼接，避免缝合感
- 融合后必须重跑 `Human Feel & Flow Review`，确认没有因为拼接造成姿态断裂、主线断裂或表演性活人感
...
5. Human Feel 复核（确认 `03-Reviews/Human-Feel-Review.md` 已通过；未通过时回到失败模块或 Momentum Map，不直接升格 final）
...
只要这篇文章存在“用户要先审、先改、再决定是否发布”的可能，就默认视为会进入“人工修改后再反哺系统”的链路。在本地协作写作场景里，默认执行 baseline 冻结，除非用户明确说“不需要保留 baseline”。

当本轮已经产出完整可改稿正文，但你准备先把稿子交给用户人工修改，而不是立刻结束在正式发布态时，执行 `human-edit handoff`：

- 将当前 `草稿_final.candidate.md` **复制**到：`草稿_final.md`
- 再将当前 `草稿_final.md` **复制**到：`03-Reviews/草稿_final.ai-baseline.md`
- 这一步的目标是同时准备好“用户可直接修改的工作稿”和“只读 baseline 证据稿”
- **不允许**为了生成 baseline 让 Agent 再写一遍文章
- 后续人工修改默认只发生在 `草稿_final.md`

如果这篇文章已经进入正式 final 交付阶段，在交付 `草稿_final.md` 时，也要确保 baseline 已冻结：
...
- 如果本轮以“可供用户继续人工改稿”的状态交付，则必须同时存在：
  - `草稿_final.candidate.md`
  - `草稿_final.md`
  - `03-Reviews/草稿_final.ai-baseline.md`
- 如果缺少其中任意一个，不算完成 handoff。
...
- Anti AI Mechanics（运行时可选）：`{selected_profile_dir}/anti-ai-mechanics.md`
- Article Archetypes（运行时可选）：`{selected_profile_dir}/article-archetypes.md`
- Opening / Question / Transition Moves（运行时可选）：`{selected_profile_dir}/opening-question-transition-moves.md`
- Anti-patterns（运行时可选）：`{selected_profile_dir}/anti-patterns.md`
- Rewrite Delta Examples（二稿 / 回流可选）：`{selected_profile_dir}/rewrite-delta-examples.md`
- Source Quality Notes（维护审计可选）：`{selected_profile_dir}/source-quality-notes.md`
...
- Human-only Brief Slots：`references/human-only-brief-slots.md`
- Article Gravity Gate：`references/article-gravity-gate.md`
- Reader Capture Audit：`references/reader-capture-audit.md`
- Reader Momentum Map：`references/reader-momentum-map.md`
- Human Feel & Flow Review：`references/human-feel-flow-review.md`
- Manual Revision Diff Protocol：`references/manual-revision-diff-protocol.md`
...
融合后将再次过 Voice + Form + Integrity + Redundancy 四检，并补 Human Feel & Flow Review，然后做 Compression Pass。"
...
| 人工改稿 diff 复盘 | 同时存在 `03-Reviews/草稿_final.ai-baseline.md` 与人工修改后的 `草稿_final.md` | 按 `references/manual-revision-diff-protocol.md` 生成候选规则卡片与 review 文件，再由用户确认是否回流到 profile 资产 |

更新流程：

1. 识别场景并提出更新建议
2. 如果是人工改稿 diff 复盘，先按 `references/manual-revision-diff-protocol.md` 生成并读取 `03-Reviews/人工改稿-diff复盘.md` 的候选规则与推荐回流位置
3. 用户确认后再修改对应文件，不允许跳过确认直接写回 profile 真相源
4. 说明新增规则对后续写作的影响

一句话触发规则：

- 用户只要说「我改完了，按写作分身回流协议做 diff 复盘」或语义等价的话，就默认进入“人工改稿 diff 复盘”
- 语义等价示例：
  - 「我改完了，复盘这次人工改稿并给我回流建议」
  - 「对这版做人工改稿 diff，看看哪些该写回 profile」
  - 「按 writing clone profile 把这次改稿沉淀掉」
- 进入后默认动作：
  - 读取 `03-Reviews/草稿_final.ai-baseline.md` 与 `草稿_final.md`
  - 生成 `03-Reviews/人工改稿-diff复盘.md`
  - 输出候选规则、目标回流文件、风险等级、推荐是否写回
  - **不自动写回长期 profile 资产**，但必须把写回建议整理好，等待用户一句确认
...
- `opening-question-transition-moves.md`
- `anti-patterns.md`
...
- **Anti AI Mechanics（运行时）**：见 `{selected_profile_dir}/anti-ai-mechanics.md`
- **Article Archetypes（运行时）**：见 `{selected_profile_dir}/article-archetypes.md`
- **Opening / Question / Transition Moves（运行时）**：见 `{selected_profile_dir}/opening-question-transition-moves.md`
- **Anti-patterns（运行时）**：见 `{selected_profile_dir}/anti-patterns.md`
- **Rewrite Delta Examples（二稿 / 人工改稿回流）**：见 `{selected_profile_dir}/rewrite-delta-examples.md`
- **Source Quality Notes（维护审计）**：见 `{selected_profile_dir}/source-quality-notes.md`
...
- **Human-only Brief Slots**：见 [references/human-only-brief-slots.md](references/human-only-brief-slots.md)
- **Article Gravity Gate**：见 [references/article-gravity-gate.md](references/article-gravity-gate.md)
- **Reader Capture Audit**：见 [references/reader-capture-audit.md](references/reader-capture-audit.md)
- **Reader Momentum Map**：见 [references/reader-momentum-map.md](references/reader-momentum-map.md)
- **Human Feel & Flow Review**：见 [references/human-feel-flow-review.md](references/human-feel-flow-review.md)
- **Manual Revision Diff Protocol**：见 [references/manual-revision-diff-protocol.md](references/manual-revision-diff-protocol.md)
