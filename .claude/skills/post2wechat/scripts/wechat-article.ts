...
async function copyHtmlFromBrowser(
  ...
  if (contentImages.length > 0) {
    console.log(
      "[wechat] Replacing img tags with placeholders for browser paste...",
    );
    const replacements = contentImages.map((img) => ({
...
function parseHtmlMeta(htmlPath: string): {
  ...
  let author = "";
  const authorMatch =
    content.match(/<meta\s+name=["']author["']\s+content=["']([^"']+)["']/i) ||
    ...
    if (fmMatch) {
      ...
      for (const line of lines) {
        ...
        if (colonIdx > 0) {
          ...
          if ((key === "description" || key === "summary") && !summary)
            summary = value;
        }
      }
    }
...
async function removeExtraEmptyLineAfterImage(
  session: ChromeSession,
): Promise<boolean> {
  const removed = await evaluate<boolean>(
    session,
    `
    (function() {
      const editor = document.querySelector('.ProseMirror');
...
export async function postArticle(options: ArticleOptions): Promise<void> {
  ...
  let chrome: ReturnType<typeof import("node:child_process").spawn> | null =
    null;
  ...
  if (portToTry) {
    ...
    if (existing) {
      ...
      console.log(
        `[cdp] Port ${portToTry} not available, launching new Chrome...`,
      );
  ...
  try {
    ...
    if (!chrome) {
      ...
      const allTargets = await cdp.send<{
        targetInfos: Array<{ targetId: string; url: string; type: string }>;
      }>("Target.getTargets");
      const loggedInTab = allTargets.targetInfos.find(
        (t) =>
          t.type === "page" &&
          t.url.includes("mp.weixin.qq.com") &&
          t.url.includes("token="),
      );
      const wechatTab =
        loggedInTab ||
        allTargets.targetInfos.find(
          (t) => t.type === "page" && t.url.includes("mp.weixin.qq.com"),
        );

      if (wechatTab) {
        console.log(
          `[wechat] Reusing existing tab: ${wechatTab.url.substring(0, 80)}...`,
        );
        const { sessionId: reuseSid } = await cdp.send<{ sessionId: string }>(
          "Target.attachToTarget",
          { targetId: wechatTab.targetId, flatten: true },
        );
        ...
        const currentUrl = await evaluate<string>(
          session,
          "window.location.href",
        );
        if (!currentUrl.includes("/cgi-bin/home")) {
          console.log("[wechat] Navigating to home...");
          const homeUrl = await evaluate<string>(
            session,
            `
              (function() {
                const current = new URL(window.location.href);
                const target = new URL('${WECHAT_URL}cgi-bin/home');
                target.searchParams.set('t', 'home/index');
    ...
    const menuReady = await waitForElement(
      session,
      ".new-creation__menu",
      20_000,
    );
    if (!menuReady) {
      ...
      const editorReady = await evaluate<boolean>(
        session,
        `!!document.querySelector('#title') && !!document.querySelector('.ProseMirror')`,
      );
      if (currentUrl.includes("/cgi-bin/appmsg") && editorReady) {
        editorSessionReady = true;
        console.log(
          "[wechat] Home menu unavailable; falling back to current editor tab.",
        );
      } else {
        throw new Error("Home page menu did not load");
      }
    }
    ...
    if (effectiveTitle) {
      const actualTitle = await evaluate<string>(
        session,
      ...
      if (actualTitle === effectiveTitle) {
        ...
        console.warn(
          `[wechat] Title verification failed. Expected: "${effectiveTitle}", got: "${actualTitle}"`,
        );
      }
    }
    ...
    if (effectiveHtmlFile && fs.existsSync(effectiveHtmlFile)) {
      ...
      const editorHasContent = await evaluate<boolean>(
        session,
        `
        (function() {
          const editor = document.querySelector('.ProseMirror');
      ...
      if (editorHasContent) {
        ...
        console.warn(
          "[wechat] Body content verification failed: editor appears empty after paste.",
        );
      }
      ...
      if (editorHasContent) {
        ...
        console.warn(
          "[wechat] Body content verification failed: editor appears empty after typing.",
        );
      }
    }
    ...
    if (effectiveSummary) {
      console.log(
        `[wechat] Filling summary (after content paste): ${effectiveSummary}`,
      );
      await evaluate(
        session,
        `
      ...
      const actualSummary = await evaluate<string>(
        session,
      ...
      if (actualSummary === effectiveSummary) {
        ...
        console.warn(
          `[wechat] Summary verification failed. Expected: "${effectiveSummary}", got: "${actualSummary}"`,
        );
      }
    }
    ...
    if (submit) {
      ...
      const submitResult = await evaluate<string>(
        session,
        `
        (function() {
          ...
          for (const btn of allBtns) {
            ...
            if (text === '保存为草稿' || text === '保存') {
              btn.click();
              return 'clicked:' + text;
            }
          }
          const nestedBtn = document.querySelector('#js_submit button');
          if (nestedBtn) {
            nestedBtn.click();
            return 'clicked:#js_submit button';
          }
      ...
      const toastMessages = await evaluate<string[]>(
        session,
        `
        Array.from(document.querySelectorAll('.weui-desktop-toast, [class*=toast]'))
          .map((el) => el.textContent?.trim())
          .filter((text) => !!text)
      `,
      );
      const savedUrl = await evaluate<string>(session, "window.location.href");
      const hasDraftId = /[?&]appmsgid=\d+/.test(savedUrl);
      if (toastMessages.length > 0) {
        console.log(`[wechat] Toast messages: ${toastMessages.join(" | ")}`);
        console.log("[wechat] Draft save signal detected.");
      } else if (hasDraftId) {
        console.log(`[wechat] Draft save confirmed via URL: ${savedUrl}`);
      } else {
        console.log("[wechat] Waiting for save confirmation...");
        await sleep(5000);
        const delayedUrl = await evaluate<string>(
          session,
          "window.location.href",
        );
        if (/[?&]appmsgid=\d+/.test(delayedUrl)) {
          console.log(
            `[wechat] Draft save confirmed via delayed URL: ${delayedUrl}`,
          );
        } else {
          throw new Error(
            "Draft save not confirmed: no toast and no appmsgid in URL",
          );
        }
      }
    } else {
      console.log(
        "[wechat] Article composed (preview mode). Add --submit to save as draft.",
      );
    }
...
Options:
  ...
  --theme <name>     Theme for markdown (dapeng-editorial, default, grace, simple, apple, byte, notion, claude)
...
Examples:
  ...
  npx -y bun wechat-article.ts --markdown article.md --theme notion --submit
