...
async function checkChrome(): Promise<void> {
  ...
  if (chromePath) {
    ...
    log(
      "Chrome",
      false,
      "Not found. Set WECHAT_BROWSER_CHROME_PATH env var or install Chrome.",
    );
  }
}
    ...
    if (exists) {
      ...
      try {
        ...
        log(
          "Profile dir",
          false,
          `Cannot create: ${e instanceof Error ? e.message : String(e)}`,
        );
      }
    }
  }
...
async function checkAccessibility(): Promise<void> {
  if (process.platform !== "darwin") {
    log(
      "Accessibility",
      true,
      `Skipped (not macOS, platform: ${process.platform})`,
    );
    return;
  }
...
`;
    ...
    const genResult = spawnSync("swift", [genScript, testPng], {
      stdio: "pipe",
      timeout: 30_000,
    });
    if (genResult.status !== 0) {
      log(
        ...
        false,
        `Cannot create test image: ${genResult.stderr?.toString().trim()}`,
      );
      return;
    }
...
`;
    ...
    const clipResult = spawnSync("swift", [clipScript, testPng], {
      stdio: "pipe",
      timeout: 30_000,
    });
    if (clipResult.status === 0) {
      log(
        "Clipboard copy (image)",
        true,
        "Can copy image to clipboard via Swift/AppKit",
      );
    } else {
      log(
        "Clipboard copy (image)",
        false,
        `Failed: ${clipResult.stderr?.toString().trim()}`,
      );
    }
  } finally {
...
async function checkPasteKeystroke(): Promise<void> {
  if (process.platform === "darwin") {
    ...
    if (result.status === 0) {
      log(
        "Paste keystroke (osascript)",
        true,
        "System Events can send keystrokes",
      );
    } else {
      const stderr = result.stderr?.toString().trim() || "";
      log(
    ...
    if (xdotool.status === 0) {
      ...
      log(
        "Paste keystroke",
        false,
        "No tool found. Install xdotool (X11) or ydotool (Wayland).",
      );
    }
...
async function checkBun(): Promise<void> {
  const result = spawnSync("npx", ["-y", "bun", "--version"], {
    stdio: "pipe",
    timeout: 30_000,
  });
  if (result.status === 0) {
    ...
    log(
      "Bun runtime",
      false,
      "Cannot run bun. Install: curl -fsSL https://bun.sh/install | bash",
    );
  }
}
...
async function checkApiCredentials(): Promise<void> {
  ...
  if (!found) {
    warn(
      "API credentials",
      "Not found. Required for API publishing method. Run the skill to set up via guided flow.",
    );
  }
}
...
async function checkRunningChromeConflict(): Promise<void> {
  ...
  if (pids.length > 0) {
    warn(
      ...
      `${pids.length} Chrome process(es) detected. The skill uses --user-data-dir for isolation, so this is safe.`,
    );
  } else {
...
async function main(): Promise<void> {
  console.log("=== post2wechat: Permission & Environment Check ===\n");
