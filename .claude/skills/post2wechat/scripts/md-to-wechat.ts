...
export async function convertMarkdown(
  ...
  const theme = options?.theme ?? "dapeng-editorial";
  ...
  let summary =
    frontmatter.summary || frontmatter.digest || frontmatter.description || "";
  ...
  const modifiedBody = bodyWithoutTitle.replace(
    /!\[([^\]]*)\]\(([^)]+)\)/g,
    (match, alt, src) => {
...
Options:
  ...
  --theme <name>      Theme name (dapeng-editorial, default, grace, simple, apple, byte, notion, claude)
  --help              Show this help
